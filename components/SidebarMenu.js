...
const SidebarContent = ({ sidebarTheme, sidebarMode, sidebarIcons, collapsed, router }) => {
  ...
  return (
    <>
      <Inner>
        {!state.mobile && (
          <Sider width={240} className={`bg-${sidebarTheme}`} theme={sidebarTheme} collapsed={collapsed}>
            {menu}
          </Sider>
        )}
        ...
        >
          <List.Item
            actions={[
              <Switch size='small' checked={!!state.boxed} onChange={(checked) => dispatch({ type: 'boxed' })} />,
            ]}
          >
            <span
              css={`
